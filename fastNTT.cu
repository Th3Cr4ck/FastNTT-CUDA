...
__global__ void ntt_kernel(uint32_t *d_A, const uint32_t *twiddles,
...
__global__ void ntt_kernel_segmentado(uint32_t *d_A,
...
__global__ void intt_kernel(uint32_t *d_A, const uint32_t *twiddles_inv,
                            const uint32_t *stage_offsets,
  ...
  for (uint32_t len = n >> 1; len > 0; len >>= 1, loop_stage++) {
...
int main() {
  ...
  // Realizar NTT
  ntt_kernel_segmentado<<<1, N_DEF / 2>>>(d_A, d_twiddles, d_stage_offsets);
